...
Page({
  ...
  //获取初始化数据
  async getInitData(){
    let placeholderContentData = await app.$API.reqSearchDefaultWord();
    // 不同榜单，要请求不同数据再合并
    let hostListData = await app.$API.reqSearchDetailList();
    this.setData({
      placeholderContent: placeholderContentData.data.showKeyword,
      hotList: {list:hostListData.data,name:'热搜榜'}
    })
  },
  ...
  async getSearchListData(){
    ...
    let searchListData = await app.$API.reqSearchList({keywords: searchContent, limit: 10});
    this.setData({
